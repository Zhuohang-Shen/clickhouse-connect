...
The server materializes the CTE only when the keyword and the `enable_materialized_cte` setting are both present, so set the setting on the statement, connection, or engine as shown in [Per-query settings](#sqlalchemy-per-query-settings). Materialized CTEs require ClickHouse 26.3 or later.
