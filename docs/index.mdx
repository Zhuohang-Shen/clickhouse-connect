...
- The optional Cython extensions in `clickhouse_connect.driverc` accelerate common serialization, conversion, and buffering paths. A pure Python path remains available when explicitly requested for platforms where the extensions cannot be built.
...
The package includes compiled wheels where available. Source builds compile the Cython extensions by default and fail if the build environment cannot compile them. PyArrow is supported on Python 3.10 through 3.14. Python 3.14 requires PyArrow 22 or later.
...
* Change to the project root and run `pip install .`. The build system installs Cython automatically and requires a working C compiler and Python development headers.

To request a slower pure Python source build explicitly, set `CLICKHOUSE_CONNECT_SKIP_CYTHON=1`:

```bash
CLICKHOUSE_CONNECT_SKIP_CYTHON=1 pip install .
```
