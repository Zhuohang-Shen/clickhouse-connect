...
- Source builds now fail when the Cython extensions cannot be prepared or compiled instead of silently retrying as a slower pure Python build. Set `CLICKHOUSE_CONNECT_SKIP_CYTHON=1` to request a pure Python build explicitly. Closes [#994](https://github.com/ClickHouse/clickhouse-connect/issues/994).
