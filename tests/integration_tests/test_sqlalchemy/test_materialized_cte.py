...
def test_materialized_cte_without_setting_still_runs(test_engine: Engine, book: Table):
    """The keyword is valid SQL on its own; the server just leaves the CTE inlined."""
    ranked = ranked_cte(book, materialized=True)
    stmt = sa_select(func.count()).select_from(ranked)
